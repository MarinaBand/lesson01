...
document.addEventListener('mousemove', function(event){
    ...
    $(".cc__floating").css("transform","rotateX("+card_y/1+"deg) rotateY("+card_x+"deg)");
    $(".cc__floating").css("box-shadow",-card_x+"px "+card_y/1+"px 55px rgba(0, 0, 0, .55)");
    $(".cc-svg").css("filter","drop-shadow("+-shadow_x+"px "+shadow_y/1+"px 4px rgba(0, 0, 0, .6))");
    $(".cc-text").css("text-shadow",-text_shadow_x+"px "+text_shadow_y/1+"px 6px rgba(0, 0, 0, .8)");
});
...
$(function(){
    setTimeout(function(){
        $(".credit-card__section").removeClass("active");
    }, 2200);
});